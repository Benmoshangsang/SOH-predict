...
def extract_VIT_capacity(x_datasets, y_datasets, seq_len, hop, sample, v=False, II=False, t=False, c=False, flip=False, slicing=False):
    V = []
    I = []
    T = []
    C = []
...
#根据放电数据的长度 (y_len)、序列长度 (seq_len) 和步长 (hop) 计算出可以生成的数据序列的数量。这里使用的公式 np.int32(np.floor((y_len - seq_len - 1) / hop)) + 1 考虑了每个序列包含的周期数和在生成连续序列时每次跳过的周期数。
    for x_data, y_data in zip(x_datasets, y_datasets):
        ...
        if slicing:
            y_slicing = int(y_len * 0.6)
            y_start = np.random.randint(1, y_len-y_slicing)
            y_df = y_df[y_start:y_start+y_slicing]
            y_len = y_slicing
